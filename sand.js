...
const settings = {
  ...
  startText: "NAPLES GOLD BUYERS",
  hiddenText: "Gold · Silver · Coins · Estate Jewelry",
...
function buildCodepenText() {
  ...
  const fontSize = Math.min((w - 40) / 10.5, h * 0.14, 96);
...
function updatePhase(dt) {
  ...
  if (phase === "codepen") {
    if (phaseTime >= settings.initialHoldSeconds) releaseCodepen();
  }
...
function tick(now) {
  ...
  if (phase !== "reform" && phase !== "hiddenHold" && phase !== "frozen") {
